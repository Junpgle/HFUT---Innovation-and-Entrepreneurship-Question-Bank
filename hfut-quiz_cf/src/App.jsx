    ...
    const parseExcelData = (rows, lectureId, lectureName) => {
        ...
        useEffect(() => {
            const checkVersion = async () => {
                try {
                    // 如果您在后端实现了 systemConfig 接口，请解开下面注释
    ...
    const handleImportProgress = (event) => {
        ...
        useEffect(() => {
            ...
            const fetchCount = async () => {
                try {
                    // ✅ 改动：调用 api.getOnlineCount
                    const res = await api.getOnlineCount();
        ...
        }, [currentUser]); // 去掉了 currentMode 依赖，简化逻辑
