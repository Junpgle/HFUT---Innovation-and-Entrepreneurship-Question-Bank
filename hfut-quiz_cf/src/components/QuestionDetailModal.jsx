        ...
        <div className="sticky top-0 bg-white border-b border-slate-100 p-6 flex justify-between items-start z-10">
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <span className="text-xs px-2 py-1 bg-red-50 text-red-700 rounded font-bold">错误 {rankInfo.errorCount} 次 · 错误率 {rankInfo.errorRate}%</span>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full transition-colors ml-4">
        ...
        <div className="p-6">
          ...
          <div className="space-y-3 mb-6">
            {options.map((opt, i) => {
              ...
              return (
                <div key={i} className={`p-4 rounded-xl border-2 text-sm flex gap-3 transition-all ${isCorrect ? 'bg-green-50 border-green-300 text-green-900 shadow-sm' : isFrequentlyWrong ? 'bg-red-50 border-red-300 text-red-900 shadow-sm' : 'bg-white border-slate-100 text-slate-600'}`}>
                  <span className={`font-bold shrink-0 ${isCorrect ? 'text-green-700' : isFrequentlyWrong ? 'text-red-700' : 'text-slate-400'}`}>{optionLetter}.</span>
                  <span className="flex-1">{opt}</span>
                  {isCorrect && <div className="flex items-center gap-1 shrink-0"><CheckCircle size={18} className="text-green-600" /><span className="text-xs font-bold text-green-700">正确答案</span></div>}
                  {isFrequentlyWrong && <div className="flex items-center gap-1 shrink-0"><XCircle size={18} className="text-red-600" /><span className="text-xs font-bold text-red-700">易错项 {selectionRate}%</span></div>}
                </div>
              );
            })}
          </div>
