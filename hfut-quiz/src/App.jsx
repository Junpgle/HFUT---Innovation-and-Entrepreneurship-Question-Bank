...
function App() {
    ...
    const performSearch = (keyword = searchKeyword) => {
        ...
        let results = allQs.filter(q => {
            // 搜索题目内容、选项和解析
            const questionMatch = q.question.toLowerCase().includes(lowerKeyword);
            const optionsMatch = q.options.some(opt => opt.toLowerCase().includes(lowerKeyword));
            const explanationMatch = q.explanation.toLowerCase().includes(lowerKeyword);
