...
AV.Cloud.define('secureSync', async (request) => {
  ...
  // =========================================================
  // 🔥【补救逻辑开始】自动统计故障期间的错题并补录到排行榜
  // =========================================================
  try {
    // 设定故障开始时间：在此时间之后产生的错题才会被补录
    const OUTAGE_START_TIME = new Date('2026-01-17T18:52:51.000+08:00');

    // 从上传的 history 中筛选出需要补录的数据
    const history = Array.isArray(params.history) ? params.history : [];
    const pendingStats = {}; // 内存聚合: { 'L1-5': 2 } 代表题号和错误次数

    history.forEach(h => {
      // 筛选条件：1.是答题操作 2.做错了 3.有题号 4.是在故障时间之后做的
      if (h.action === 'answer' && h.isCorrect === false && h.questionId) {
        const recordTime = new Date(h.timestamp);
        if (recordTime > OUTAGE_START_TIME) {
          pendingStats[h.questionId] = (pendingStats[h.questionId] || 0) + 1;
        }
      }
    });
    ...
    if (questionIdsToUpdate.length > 0) {
      console.log(`[Stats Recovery] User ${currentUser.id} uploading ${questionIdsToUpdate.length} wrong questions.`);
      ...
      const dirtyObjects = []; // 待保存的对象列表

      // 3.2 遍历并合并数据
      for (const qId of questionIdsToUpdate) {
        const errorCountToAdd = pendingStats[qId];
        let statObj = statsMap.get(qId);

        if (!statObj) {
          // 如果数据库里还没这题的记录（比如是新题），新建一个
          const WrongQuestionStats = AV.Object.extend('WrongQuestionStats');
          statObj = new WrongQuestionStats();
          statObj.set('questionId', qId);
          statObj.set('questionTitle', '补录数据-等待刷新'); // 暂时没有标题，下次正常答题时会修正
        ...
        // 原子增加错误次数和总尝试次数
        statObj.increment('errorCount', errorCountToAdd);
        statObj.increment('totalAttempts', errorCountToAdd);

        dirtyObjects.push(statObj);
      }

      // 3.3 批量保存 (SaveAll) - 极其节省 API 调用，N条数据只消耗1次请求
      if (dirtyObjects.length > 0) {
        await AV.Object.saveAll(dirtyObjects, { useMasterKey: true });
        console.log(`[Stats Recovery] Successfully patched ${dirtyObjects.length} records.`);
      }
    }
  } catch (err) {
    // ⚠️ 关键：补录逻辑报错绝不能影响用户正常的“进度保存”
    // 所以这里只打印日志，不抛出异常，让程序继续往下走
    console.error('[Stats Recovery Failed]', err);
  }
  // =========================================================
  // 🔥【补救逻辑结束】
  // =========================================================
